...
func (csp *CSP) AttachDisk(vmDiskPath string, storagePolicyName string, nodeName k8stypes.NodeName) (diskUUID string, err error) {
	...
	glog.V(3).Infof("vSphere Cloud Provider attaching disk %+v on node %s", vmDiskPath, nodeName)
...
func (csp *CSP) DetachDisk(volPath string, nodeName k8stypes.NodeName) error {
	// TODO: Modify vmdiskPath to all input options in the log next line.
	glog.V(3).Infof("vSphere Cloud Provider detaching disk %+v from node %s", volPath, nodeName)
	node, err := csp.nodeManager.GetNode(convertToString(nodeName))
	if err != nil {
		glog.Errorf("Cannot get node %s from nodemanager for detaching disk %+v with error %+v",
			nodeName, volPath, err)
		return err
	}
	vc, err := csp.virtualCenterManager.GetVirtualCenter(node.VirtualCenterHost)
	if err != nil {
		glog.Errorf("Cannot get virtual center %s from vitualcentermanager for detaching disk %+v with error %+v", node.VirtualCenterHost, volPath, err)
		return err
	}
	...
	if err != nil {
		glog.Errorf("Failed to dettach disk %s with err %+v", volumeID, err)
		return err
	}
	return nil
}
...
func (csp *CSP) DiskIsAttached(volPath string, nodeName k8stypes.NodeName) (bool, error) {
	glog.V(3).Infof("vSphere Cloud Provider checking disk is attached %s to node %s", volPath, nodeName)
	nodeVolumes := make(map[k8stypes.NodeName][]string)
	nodeVolumes[nodeName] = append(nodeVolumes[nodeName], volPath)
	volumesAttached, err := csp.DisksAreAttached(nodeVolumes)
	if err != nil {
		glog.Errorf("Failed to check disk is attached %s to node %s with error %+v", volPath, nodeName, err)
		return false, err
	}
	return volumesAttached[nodeName][volPath], nil
}
...
func (csp *CSP) DisksAreAttached(nodeVolumes map[k8stypes.NodeName][]string) (map[k8stypes.NodeName]map[string]bool, error) {
	...
	attachedVolumes := make(map[k8stypes.NodeName]map[string]bool)
	for node, volumes := range volumesAreAttached {
		k8snode := convertToK8sType(node)
		if _, ok := attachedVolumes[k8snode]; !ok {
			attachedVolumes[k8snode] = make(map[string]bool)
		}
		for volumeID, attached := range volumes {
			attachedVolumes[k8snode][volumeID.ID] = attached
		}
	}
...
func (csp *CSP) CreateVolume(volumeOptions *vclib.VolumeOptions) (volumePath string, err error) {
	glog.V(3).Infof("vSphere Cloud Provider creating disk %+v", volumeOptions)
	vc, err := csp.virtualCenterManager.GetVirtualCenter(csp.cfg.Workspace.VCenterIP)
	if err != nil {
		glog.Errorf("Cannot get virtual center %s from vitualcentermanager for creating disk %+v with error %+v",
			csp.cfg.Workspace.VCenterIP, volumeOptions.Name, err)
		return "", err
	}
	...
	glog.V(5).Infof("vSphere Cloud Provider creating volume %s with create spec %+v", volumeOptions.Name, createSpec)
	volumeID, err := cspvolumes.GetManager(vc).CreateVolume(createSpec)
	if err != nil {
		glog.Errorf("Failed to create disk %s with error %+v", volumeOptions.Name, err)
		return "", err
	}
...
func (csp *CSP) DeleteVolume(vmDiskPath string) error {
	// TODO: Modify vmdiskPath to all input options in the log next line.
	glog.V(3).Infof("vSphere Cloud Provider deleting volume %+v", vmDiskPath)
	vc, err := csp.virtualCenterManager.GetVirtualCenter(csp.cfg.Workspace.VCenterIP)
	if err != nil {
		glog.Errorf("Cannot get virtual center %s from vitualcentermanager for delete disk %+v with error %+v",
			csp.cfg.Workspace.VCenterIP, vmDiskPath, err)
		return err
	}
	...
	glog.V(5).Infof("vSphere Cloud Provider deleting volume %s with delete spec %+v", vmDiskPath, deleteSpec)
	err = cspvolumes.GetManager(vc).DeleteVolume(deleteSpec)
	if err != nil {
		glog.Errorf("Failed to delete disk %s with error %+v", vmDiskPath, err)
		return err
	}
	// TODO: Return VolumeID
	return nil
